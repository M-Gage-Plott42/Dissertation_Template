...
\titlespacing*{\chapter}{0pt}{0.75in}{0.05in}
...
\makeatletter
  ...
  \pretocmd{\l@figure}{%
    \iftoggle{FirstFigureEntry}{%
      % first entry
      \vskip-12pt\togglefalse{FirstFigureEntry}%
    }{%
...
\newcommand{\ManuscriptTitle}{TITLE OF THESIS OR DISSERTATION IN ALL CAPS}
...
\newcommand{\UTCTitlePageOuterGap}{1.5in}
\newcommand{\UTCTitlePageDegreeGap}{0.5in}
\newcommand{\UTCTitlePageDateGap}{0.25in}
...
\begin{center}
\ManuscriptTitle\par
% UTC sample title page keeps the title at the 2 in opener line, but the
% title-to-"By" and name-to-body spacer family is substantially larger than
% the default chapter/list opener gaps.
\vspace{\UTCTitlePageOuterGap}
By\par
\AuthorFullName\par
\vspace{\UTCTitlePageOuterGap}
\begin{singlespace}
...
of \DegreeName\\[\UTCTitlePageDegreeGap]
The University of Tennessee at Chattanooga\\
Chattanooga, Tennessee\\[\UTCTitlePageDateGap]
\GradMonthYear
\end{singlespace}
\end{center}
\restoregeometry
...
{\singlespacing
  % UTC: .lot starts with \addvspace; compensate so first entry sits below the standard title gap.
  \vspace*{-2\baselineskip}
  \UTCstarttoc{lot}
}
\clearpage
...
{\singlespacing
  % UTC: .lof starts with \addvspace; compensate initial entry offset.
  \vspace*{-\baselineskip}
  \UTCstarttoc{lof}
}
...
\include{chapters/ch02_lit_review}
...
\include{chapters/ch07_future_work}

\renewcommand{\bibname}{REFERENCES}
\defbibheading{utcrefs}[REFERENCES]{%
  \chapter*{#1}%
  \addcontentsline{toc}{chapter}{#1}%
  \vspace*{-0.25\baselineskip}%
}
\nocite{*}
\printbibliography[heading=utcrefs,title={REFERENCES}]
